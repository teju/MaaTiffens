		...
		if (!isInEditMode()) {
			...
			if (attrs != null) {
				TypedArray a = context.obtainStyledAttributes(attrs,
						R.styleable.maatiffens);

				// Set the colors from the attributes
				int loadingMessageColor = a.getColor(
						R.styleable.maatiffens_loadingMessageColor, context
								.getResources().getColor(R.color.DarkGray));
				int errTitleColor = a.getColor(
						R.styleable.maatiffens_loadingErrorTitleColor, context
								.getResources().getColor(R.color.Black));
				int errMessageColor = a.getColor(
						R.styleable.maatiffens_loadingErrorMessageColor,
				...
				String loadingMessage = a.getString(R.styleable.maatiffens_loadingMessage);
				if (loadingMessage != null) {
					tvLoading.setText(loadingMessage);
				}

				String retryButtonText = a.getString(R.styleable.maatiffens_loadingRetryButtonText);
				if (retryButtonText != null) {
					btnRetry.setText(retryButtonText);
				}

				boolean useNilaGif = a.getBoolean(R.styleable.maatiffens_loadingGifNila, false);
				if(useNilaGif){
//					Glide.with(context)
//							.asGif()
...
//					tvLoading.setVisibility(View.GONE);
				...
				noNetworkMessage = a.getString(R.styleable.maatiffens_loadingErrorNetworkTitle);
				unknownResponseMessage = a.getString(R.styleable.maatiffens_loadingErrorUnknownResponseMessage);
				noInternetMessage = a.getString(R.styleable.maatiffens_loadingErrorNoInternetMessage);

				if (noNetworkMessage == null) {
					noNetworkMessage = getContext().getString(R.string.maatiffens__network_error);
				}

				if (noInternetMessage == null) {
					noInternetMessage = getContext().getString(R.string.maatiffens__no_internet);
				}

				if (unknownResponseMessage == null) {
					unknownResponseMessage = getContext().getString(R.string.maatiffens__unknown_response);
				}
	...
	public void showLoadingV3() {
		this.clearAnimation();
		this.setBackgroundColor(ContextCompat.getColor(getContext(), R.color.DarkGray));
